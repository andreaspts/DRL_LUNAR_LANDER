...
# Specifying the learning agents parameters
BUFFER_SIZE = int(1e6)  # replay buffer size
BATCH_SIZE = 64         # minibatch size
GAMMA = 0.99            # discount factor
TAU = 1e-3              # for soft update of target parameters
LR = 5e-4               # learning rate 
UPDATE_EVERY = 4        # how often (how many steps) to update the network
...
class Agent():
    ...
    def targets(self,gamma,rewards,next_states,dones):
        ...
        y = torch.add(rewards,torch.mul(torch.max(q,dim=1,keepdim=True)[0],gamma)) * (1 - dones)
